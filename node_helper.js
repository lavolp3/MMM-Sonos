...
module.exports = NodeHelper.create({
        ...
        if (!this.asyncDevice) {
            this.asyncDevice = this.discovery.discover().then(device => {
                listener.on('ZonesChanged', () => {
                    self.log(`Zones have changed. Rediscovering all groups ...`);
                    this.discoverGroups();
                });
    ...
    setListeners: function (groups) {
        ...
        groups.forEach(group => {
            self.log(`Registering listeners for group "${group.Name}" (host "${group.host}")`);

            const sonos = group.CoordinatorDevice();
            
            sonos.on('Mute', isMuted => {
                self.log('This speaker is %s.', isMuted ? 'muted' : 'unmuted')
            });

            sonos.on('CurrentTrack', track => {
                self.log(`[Group ${group.Name} - ${group.host}] Track changed to "${track.title}" by "${track.artist}"`);
            ...
            sonos.on('Volume', volume => {
                self.log(`[Group ${group.Name} - ${group.host}] Volume changed to "${volume}"`);
            ...
            sonos.on('Muted', isMuted => {
                self.log(`[Group ${group.Name} - ${group.host}] Group is ${isMuted ? 'muted' : 'unmuted'}`);
            ...
            sonos.on('PlayState', state => {
                self.log(`[Group ${group.Name} - ${group.host}] Play state change to "${state}"`);
